<style>
<?php if(isset($IsMobile) && $IsMobile == 'yes'){}else{?>
...
.ilatc_head{font-family:goboldbolditalic!important;color:#fff;padding:10px 0;font-size:21px;text-align:center;text-shadow:0 0 15px #000;margin:0}
.phancy-scroller{margin-top:0!important}
...
.item-bx-arm:last-child { border-bottom: none; }
...
.add-cart-btn .ful-wd-btn{width:100%;font-size:16px!important;border-radius:0;background:#093 none repeat scroll 0 0;color:#fff;text-transform:uppercase;border:none;display:block;padding:8px;text-align:center;transition:all 500ms ease}

#pro-sel-pg .add-cart-btn a:hover{background:#444 none repeat scroll 0 0!important}
.pos-abst{position:relative;bottom:0;width:100%;padding:15px 5px;height:auto;background:#000;left:0;z-index:99;display:block}
.pos-abst #new-ui-add{max-width:1170px;margin:0 auto}
...
/*.pos-abst #new-ui-add select{font-size:22px!important;height:40px;padding:0 10px;-webkit-appearance:none;-moz-appearance:none;appearance:none;color:#fff;border:2px solid #fff!important;background-color:#212728!important;height:40px;margin:0;width:100%!important;background-image:url(< ?php echo SITEURL;?>bootstrap_3_3_6/img/arrow-whte.png);background-position:right center;background-position:96% center;background-repeat:no-repeat;background-size:16px}*/
...
.arrow-setng{position:fixed;bottom:0;width:100%;leftl:0;left:0;z-index:999}
.arw-ups.arw-wd{position:relative;bottom:0;z-index:555;left:0;right:0;margin:0 auto;padding:10px 0;background:#000;margin:0}
.set-arws{max-width:70px;margin:auto;position:absolute;top:-25px;left:-2px;right:0;z-index:55}
.set-arws img{margin-top:6px}
.set-arws:before{width:0;height:0;border-bottom:45px solid #000;border-left:60px solid transparent;border-right:60px solid transparent;content:"";position:absolute;left:-25px;right:0;margin:auto;bottom:-7px;z-index:-5}
...
div#select-cars_fix{display:none}
.featrs-txt p{min-height:60px}
...
.arrow-setng{position:relative;z-index:555}
}
...
.thumbnail span{position:absolute;background-color:#fff;padding:1px;left:-1000px;border:1px solid #000;color:#000;text-decoration:none;width:250%;
visibility:hidden;
}
.thumbnail span img{border-width:0;padding:2px}
<?php if( isset($IsMobile) ){}else{?>
.thumbnail:hover span{visibility:visible;top:-30px;left:120px}
<?php }?> 
</style>
...
<div class="pos-abst" id="select-cars_fix" style="display: block;">   
...
<div class="clearfix"></div> 
    <div class="car-img" id="car_pic"><?php if(isset($aList['image']) && !empty($aList['image'])){ echo $aList['image'];}?></div>
</div>
<div class="clearfix"></div>
...
<?php
$col = 'col-sm-12';
if(!empty($cList['data1']) && !empty($cList['data']) && !empty($cList['accessory']) ){ $col = 'col-sm-4'; }
elseif(!empty($cList['data1']) && !empty($cList['data']) && empty($cList['accessory']) ){ $col = 'col-sm-6'; }
elseif(!empty($cList['data1']) && empty($cList['data']) && empty($cList['accessory']) ){ $col = 'col-sm-12'; }
...
<div class="<?php echo $col;?>"><h1 class="ilatc_head">CAT-BACK VALVETRONIC EXHAUST</h1>
<div class="scrol-bx" id="demo_2">      
<?php if(isset($cList['data1']) && !empty($cList['data1'])){ echo $cList['data1'];}?>
</div></div>
<!--end of first col-->

<?php if(isset($cList['data']) && !empty($cList['data'])){?>
<div class="<?php echo $col;?>"><h2 class="ilatc_head">Headers/downpipes</h2>
<div class="scrol-bx" id="demo">      
<?php  echo $cList['data'];?>
</div></div>
<?php }?>
<!--end of second col-->
<?php if(isset($cList['accessory']) && !empty($cList['accessory'])){?>
<div class="col-sm-4"><h2 class="ilatc_head">Accessory</h2>
<div class="scrol-bx" id="accessory_list">      
<?php echo $cList['accessory'];?>
</div></div>
<?php }?>
...
<script>
		...
		jQuery(document).ready(function(){
			...
			$( "#motor" ).change(function() {
				...
				window.location.href = SITEURL+'product-exhaust-result?brand='+brand+'&model='+model+'&motor='+v;
