...
<input type="number" class="form-control" id="total" value="<?php echo @$q['t'];?>" placeholder="# Total"><span class="input-group-btn"> <button type="button" class="btn btn-info btn-flat" id="t_filter">Filter!</button> </span>
					...
					<tr role="row">
                        ...
                        <th><?php echo $this->Paginator->sort('User.first_name', '	Customer', array('escape' => false)); ?></th>
...
$(document).ready(function () {
	...
	$( "#order_status" ).change(function() { window.location.href = '<?php echo SITEURL;?>lab/labs/order_dealers?s='+this.value; });
	$( "#payment_by" ).change(function() { window.location.href = '<?php echo SITEURL;?>lab/labs/order_dealers?p='+this.value; });

	$( "#t_filter" ).click(function() {
		var t = $.trim($('#total').val());
		if(t != ''){ window.location.href = '<?php echo SITEURL;?>lab/labs/order_dealers?t='+t; }
		});
