...
class LabsController extends AppController {
    ...
    var $uses = array('User','Model','Brand','Motor','Product','EmailTemplate','World','Library','ExhaustBrand','ExhaustModel','ExhaustProduct','PromoCode',
    ...
    public function lab_vote() {
    	...
    	if(isset($this->request->query['st']) && !empty($this->request->query['st'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['Vote']['status'] == 1){ $st = 0;}
    			elseif($d['Vote']['status'] == 0){ $st = 1;}
    ...
    public function lab_quick_vote($id = null) {
    	...
    	if(!empty($this->data)){
    		...
    		if(empty($t)){ echo $s; echo "<div class='alert alert-danger'>Please enter Title</div>"; exit;}
    		elseif(empty($d)){ echo $s; echo "<div class='alert alert-danger'>Please enter description</div>"; exit;}
    		else{
    			...
    			foreach ( $this->data['VoteOption'] as $list ){
    				...
    				if(isset($list['id']) && !empty($list['id'])){}
    					...
    					if(!empty($a) && !isset($list['img']['name'])){ $f++; } 
    				}
    			}
    			
    			if( $o <2){ echo $s; echo "<div class='alert alert-danger'>Please add at least 2 options</div>"; exit;}
    			
    			/* if( $f != 0 ){ echo $s; echo "<div class='alert alert-danger'>Please add image for option</div>"; exit;} */
    			
    			$this->Vote->save($this->data);
    			if(isset($this->data['Vote']['id']) && !empty($this->data['Vote']['id'])){ $lid = $this->data['Vote']['id']; }
    			else{ $lid = $this->Vote->getLastInsertId(); }
    			$abc = array();
    			foreach ( $this->data['VoteOption'] as $list ){
    				...
    				if(isset($list['img']['name']) && !empty($list['img']['name'])){
    					...
    					if (in_array($imgExt, array('jpg','png','jpeg'))) {
    						...
    						if (!file_exists('cdn/vote')) { mkdir('cdn/vote', 0777, true); }
    			...
    			if(!empty($opt)){ $this->VoteOption->saveMany($opt);}
    ...
    public function lab_new_faq($id = null) {
    	...
    	if(!empty($id)){
    		...
    		if ($this->request->is('get')) {
    			...
    			else { $this->redirect(array('controller' => 'labs', 'action' => 'new_faq')); }
    		} else {
    ...
    public function lab_create_our_dealer($id = null) {
    	...
    	if(!empty($id)){
    		...
    		if ($this->request->is('get')) {
    			...
    			if (!empty($e)) { $this->request->data = $e; }
    			else { $this->redirect(array('controller' => 'labs', 'action' => 'our_dealer')); }
    		} else {
    ...
    public  function lab_our_dealer(){
    	...
    	if(isset($this->request->query['st']) && !empty($this->request->query['st'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['OurDealer']['status'] == 1){ $st = 0;}
    			elseif($d['OurDealer']['status'] == 0){ $st = 1;}
    ...
    public function _xls_to_shipping($file = null){
    	...
    	if(!empty($file)){
    		...
    		if(isset($data->sheets[0]['cells'][2]) && !empty($data->sheets[0]['cells'][2])){
    			...
    			}else{ echo "<div class='alert alert-danger'>State record not found</div>"; }
    
    		}else{ echo "<div class='alert alert-danger'>Record not found</div>";}
    	}
    }
    public function lab_up_xls(){
    	...
    	if ($this->request->is('ajax') && !empty($this->data)) {
    			...
    			}else{ echo "<div class='alert alert-danger'>Please selecte only XLS file</div>";}
    ...
    public function _xls_to_arr($file = null){
    	...
    	if(isset($data->sheets[0]['cells'][2]) && !empty($data->sheets[0]['cells'][2])){
    		...
    		foreach ($sAll as $k=>$list){
    			...
    			if(!empty($s)){ $sList[$f[1]] = $s['World']['id'];}
    			else{ $sList[$f[1]] = $list[1]; }
    ...
    public function lab_update_insta($id = null) {
        ...
        if( !empty($id)){
            ...
            if(empty($data)){ $this->layout = '404'; }
            $this->set('e', $data);
        }
        
        if ($this->RequestHandler->isAjax() && !empty($this->data)) {
            
            if (!file_exists('cdn/instagram')) { mkdir('cdn/instagram', 0777, true); }
            
            if (filter_var( $this->data['Instagram']['url'], FILTER_VALIDATE_URL) === false){ echo '<div class="alert alert-danger" role="alert">Please enter correct url.</div>'; }
            elseif( empty($this->data['Instagram']['user_name'])) { echo '<div class="alert alert-danger" role="alert">Please enter USER NAME.</div>'; }
            elseif( empty($this->data['Instagram']['img']['name']) ){ echo '<div class="alert alert-danger" role="alert">Please select an image.</div>'; }
            else{
...
public function lab_archive() {
    	...
    	if(isset($q['s']) && !empty($q['s'])){ $c[] = array('Order.order_status_id'=>$q['s']); }
    	if(isset($q['p']) && !empty($q['p'])){ $c[] = array('Order.payment_by'=>$q['p']); }
    	if(isset($q['t']) && !empty($q['t'])){ $c[] = array('Order.grand_total'=>$q['t']); }
    
    	 
    	$this->Order->bindModel(array('belongsTo'=>array('User','World'=>array('foreignKey'=>'shipping_country','fields'=>array('id','name'))),
    			'hasMany'=>array('OrderMessages'=>array('conditions'=>array('OrderMessages.is_new_admin'=>0),'limit'=>1))));
    	$this->paginate = array('recursive'=>1, 'conditions'=> $c ,
    ...
    public function lab_order_users_advance() {
    	...
    	if ($this->request->is('ajax')) {
    		$searchData = $this->request->data['name_value'];
    		$this->paginate = array('limit' => 100,
    				'conditions' => array( 'User.role'=>array(1,2),'Order.hide'=>0,'Order.order_status_id NOT'=>0,
    						'or' => array(
    ...
    public function lab_order_dealers_advance() {
    	...
    	if ($this->request->is('ajax')) {
    		$searchData = $this->request->data['name_value'];
    		$this->paginate = array('limit' => 100,
    				'conditions' => array( 'User.role'=>array(3),'Order.hide'=>0,'Order.order_status_id NOT'=>0,
    						'or' => array(
    								"Order.order_number" => $searchData,
    								"User.first_name" =>$searchData,
    								"User.last_name" =>$searchData,
    	...
    	if ($this->request->is('ajax') && !empty($this->data)) {
    		...
    		if($this->data['type'] == 'update' && !empty($this->data['id']) && !empty($this->data['st'])){
    		    if(in_array($this->data['st'], array(3)) && empty($this->data['tu']) && empty($this->data['tn']) ){
    		        echo $s; echo '<div class="alert alert-danger fadeIn animated">Pelease enter Tracking Number and URL.</div>'; exit;
    		    }
    				...
    				if ( !empty($data['OrderHistory']) ){
    				    foreach ($data['OrderHistory'] as $h_list){
    				        if ($this->data['st'] == $h_list['order_status_id']){
    				            echo $s; echo '<div class="alert alert-danger fadeIn animated">This Status already updated. Please select different status.</div>'; exit; }
    				    }
    				}
    				$o_item = [];
    				        ...
    				        if(in_array($this->data['st'], array(3))){
    				            ...
    				            if ( !empty($chk_pro) ){
    				                if ( $chk_pro['Product']['quantity'] >= $list['quantity'] ){
    				                    ...
    				                    echo $s; echo '<div class="alert alert-danger fadeIn animated">You don not have enough product quantity ( Product ID #'.$list['product_id'].' )</div>'; exit;
    				                }
    				            }else{ echo $s; echo '<div class="alert alert-danger fadeIn animated">Product ID #'.$list['product_id'].' not found.</div>'; exit; }
    ...
    public function lab_edit_level($id=null) {
    	...
    	if(!empty($this->data)){
    		...
    		if($this->DealerLevel->save($this->data)){ $this->Session->setFlash(__('Saved.'), 'default', array('class' => 'alert alert-success'), 'msg'); $this->redirect($this->referer());}
    		else{ $this->Session->setFlash(__('Not saved'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
    ...
    public function lab_add_extra_product() {
		...
		if(isset($this->request->query['edit'])){
			...
			if(!empty($e)){
				if(!empty($e['Product']['extra_photos'])){
					...
					$newIDS = array();
					if(!empty($pics)){ foreach ($pics as $plist){ $newIDS[] = $plist['Library']['id']; } }
					if(!empty($newIDS)){
		...
		if ($this->RequestHandler->isAjax()) {
			...
			if(empty($this->data['Product']['description'])){ echo $s; echo '<div class="alert alert-danger" role="alert"> Please enter product description.</div>'; exit;}
			else{
    ...
    public function lab_extra_product() {
    	...
    	if(isset($this->request->query['status']) && !empty($this->request->query['status'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['Product']['status'] == 1){ $st = 0;}
    			elseif($d['Product']['status'] == 0){ $st = 1;}
    	...
    	$this->paginate = array('recursive'=>-1, 'conditions'=>array('Product.type'=>4),
    ...
    public function lab_up_pro(){
    	...
    	if ($this->request->is('ajax')) {
    		$s = "<script>$('#preloader').hide();</script>";
    		if(!empty($this->data)){
    			if($this->data['type'] == 'cat-back'){ $type = 2; $f='cat_back_ids';}
	    		elseif($this->data['type'] == 'catalytic'){ $type = 3; $f='catalytic_ids';}
	    		elseif($this->data['type'] == 'accessory'){ $type = 5; $f='accessory_ids';}
	    		...
	    		}else{ echo $s; echo "<script>$.magnificPopup.close(); location.reload();</script>";}
    ...
    public function lab_add_product($id=null,$type=null) {
    	if ($this->request->is('ajax')) {
    		...
    		if(!empty($i)){
    			if($type == 'cat-back'){ $t = 2;}
    			elseif($type == 'catalytic'){ $t = 3; }
    			elseif($type == 'accessory'){  $t = 5;}
    			if(isset($t)){ 
    ...
    public function lab_all_make($id=null) {
    	...
    	if(!empty($id)){
    		...
    		if ($this->RequestHandler->isAjax()) {
    			$d = null;
    			if(empty($this->data['id'])){ $d = $this->Brand->find('count',array('conditions'=>array('Brand.name'=>$this->data['name']))); }
    			if($d > 0){ echo '<div class="alert alert-info" role="alert"> This Brand name already exist.</div>';}
    			else{
		...
		if( isset($this->request->query['status']) && !empty($this->request->query['status'])){
			...
			if(!empty($list)){
				if($list['Model']['status'] == 1){ $st = 0;}else { $st = 1; }
		...
		if ($this->RequestHandler->isAjax()) {
			...
			if( empty($this->data['brand_id'])){ echo '<div class="alert alert-info" role="alert"> Please select brand name.</div>';}
			elseif (empty($this->data['name']) ){ echo '<div class="alert alert-info" role="alert"> Please enter model name</div>'; }
			else{
	...
	public function lab_all_motor($id=null,$mid=null) {
		...
		if( isset($this->request->query['status']) && !empty($this->request->query['status'])){
			...
			if(!empty($list)){
				if($list['Motor']['status'] == 1){ $st = 0;}else { $st = 1; }
		...
		$c = array();
		if(!empty($id)){ $c[] = array('Motor.brand_id'=>$id);}
		if(!empty($mid)){ $c[] = array('Motor.model_id'=>$mid);}
		...
		if ($this->RequestHandler->isAjax()) {
			...
			$d = 0;
			if( empty($this->data['brand_id'])){ echo '<div class="alert alert-info" role="alert"> Please select brand name.</div>';}
			elseif( empty($this->data['model_id'])){ echo '<div class="alert alert-info" role="alert"> Please select model name.</div>';}
			elseif (empty($this->data['name']) ){ echo '<div class="alert alert-info" role="alert"> Please enter motor name</div>'; }
			else{
				
				if(empty($this->data['id'])){ $d = $this->Motor->find('count',array('conditions'=>array('Motor.name'=>$this->data['name'],'Motor.model_id'=>$this->data['model_id']))); }
				if($d > 0){ echo '<div class="alert alert-danger" role="alert"> This Motor name already exist.</div>';}
				else{
	...
	public function lab_new_tuning_box($brand_id=null,$model_id=null,$engine_id=null) {
		...
		if(!empty($brand_id)){ $c = array('Model.brand_id'=>$brand_id);}
		$model_list = $this->Model->find('list',array('conditions'=>$c, 'order'=>array('Model.name'=>'ASC'),'fields'=>array('id','name')));
		//ec($model_list); die;
		
		if(!empty($model_id)){ $engList= $this->Motor->find('list',array('conditions'=>array('Motor.model_id'=>$model_id),'order'=>array('Motor.name'=>'ASC'),'fields'=>array('id','name'))); }
		
		if(isset($this->request->query['edit'])){
			$e = $this->Product->find('first',array('conditions'=>array('Product.type'=>1,'Product.id'=>$this->request->query['edit'])));
			
			if(empty($e)){ $this->layout = 'lab_404'; }
			$this->set('e',$e);
		}
		...
		if ($this->RequestHandler->isAjax()) {
			...
			if(empty($this->data['Product']['description'])){ echo $s; echo '<div class="alert alert-danger" role="alert"> Please enter product description.</div>'; exit;}
			elseif(empty($this->data['Product']['img'][0]) && empty($this->data['Product']['id'])){ echo $s; echo '<div class="alert alert-danger" role="alert"> Please select at least one photo for product.</div>'; exit;}
			else{
			...
			if( empty($this->data['Product']['id']) ||  !empty($this->data['Product']['img'][0]) ){
				foreach ($this->data['Product']['img'] as $imgList){
					...
					if (in_array($imgExt, $extImg)) { $isExt++; }
					else{ $MorenoExt++; } }
			}
			...
			if( empty($this->data['Product']['id']) ||  !empty($this->data['Product']['more_photos'][0]) ){
				foreach ($this->data['Product']['more_photos'] as $imgList){
					...
					$imgFilename = $imgList['name'];
					$imgExt = strtolower( pathinfo($imgFilename, PATHINFO_EXTENSION));
					if (in_array($imgExt, $extImg)) { $MoreisExt++; }
					else{ $noExt++; } }
				}
			}
				
			if($noExt > 0 ){ echo $s; echo '<div class="alert alert-danger" role="alert"> Please upload png and jpg files only.</div>'; exit; }
			if($MorenoExt > 0 ){ echo $s; echo '<div class="alert alert-danger" role="alert"> Please upload png and jpg files only.</div>'; exit; }

			$name = $this->data['Product']['title']."-".rand(1,999);
			$slug = strtolower(Inflector::slug($name, '-'));
			if( empty($this->data['Product']['id']) ){ $this->request->data['Product']['slug'] = $slug; }
			$this->request->data['Product']['status'] = 1;
		...
		if( empty($this->data['Product']['id']) || !empty($this->data['Product']['img'][0]) ){
			foreach ($this->data['Product']['img'] as $imgList){
				...
				try {
						if (move_uploaded_file($imgList['tmp_name'], $Image_path)) { if (file_exists($Image_path)) { $r = $this->DATA->compress_image($Image_path,$Image_path,65); $imgArr[] = $iName; } }
			...
			if( empty($this->data['Product']['id']) || !empty($this->data['Product']['more_photos'][0]) ){
				foreach ($this->data['Product']['more_photos'] as $imgList){
					...
					$Image_path = 'cdn/cdnimg/'.$iName;
					try {
						if (move_uploaded_file($imgList['tmp_name'], $Image_path)) { if (file_exists($Image_path)) { $r = $this->DATA->compress_image($Image_path,$Image_path,65); $MoreimgArr[] = $iName; } }
					}catch (Exception $e) {  $Moreerr++; } }
				}
			}
			
			if($err > 0){ echo $s; echo '<div class="alert alert-danger" role="alert">Error during image uploading. please try again later.</div>'; exit; }
			else{ 
				if(!empty($imgArr)){ $this->request->data['Product']['images'] = json_encode($imgArr); } }
				
			if($Moreerr > 0){ echo $s; echo '<div class="alert alert-danger" role="alert">Error during image uploading. please try again later.</div>'; exit; }
			else{
				if(!empty($MoreimgArr)){ $this->request->data['Product']['more_photo'] = json_encode($MoreimgArr); }
			}
	...
	public function lab_tuning_box() {
		...
		if( isset($this->request->query['status']) && !empty($this->request->query['status'])){
			...
			if(!empty($list)){
				if($list['Product']['status'] == 1){ $st = 0;}else { $st = 1; }
    	...
    	if ($this->request->is('ajax')) {
    		...
    		if($this->data['type'] == 'del' && !empty($this->data['id']) && !empty($this->data['lid'])){
    			...
    			if(!empty($d)){
    				...
    				if(!empty($arr)){ $this->ItemDetail->save($arr); }
    ...
    public function lab_add_utube() {
    	...
    	if(!empty($this->data)){
    		...
    		if(empty($vi)){ $this->Session->setFlash(__('Please enter valid Youtube ID/URL'), 'default', array('class' => 'alert alert-danger'), 'msg');
    		}else{
    		...
    		if(!empty($d)){
    			...
    			}else { $vid = $this->data['ItemDetail']['vid'];}
    ...
    public function lab_del_car($id = null) {
    		...
    		if(!empty($id)){
    			...
    			if(!empty($d)){
	    			...
	    			$this->Shipping->deleteAll(array('Shipping.item_detail_id'=>$d['ItemDetail']['id']),false);
	    			$this->QualityDetail->deleteAll(array('QualityDetail.item_detail_id'=>$d['ItemDetail']['id']),false);
    			}
    			$this->redirect($this->referer());
    		}
    ...
    public function lab_all_car_details() {
    	...
    	if( isset( $this->request->query['id'] ) && !empty($this->request->query['id']) ){
    		...
    		if(!empty($d)){
    			$status = 1;
    			if($d['ItemDetail']['status'] == 1){ $status = 0;}
    	...
    	$q = $this->request->query;
    	$brand = $this->Brand->find('list',array('order'=>array('Brand.name'=>'ASC'),'fields'=>array('id','name')));
    	if(isset($q['brand']) && !empty($q['brand'])){ $c = array('Model.brand_id'=>$q['brand']); $ec[]= array('ItemDetail.brand_id'=>$q['brand']); }
    	$model_list = $this->Model->find('list',array('conditions'=>$c, 'order'=>array('Model.name'=>'ASC'),'fields'=>array('id','name')));
    	if( isset($q['model']) && !empty($q['model'])){
    		...
    		if( isset($q['motor']) && !empty($q['motor'])){ $ec[]= array('ItemDetail.motor_id'=>$q['motor']); }
    		 
    	
    	
    	$this->ItemDetail->bindModel(array('belongsTo'=>array('Brand','Model','Motor')));
    	$this->paginate = array('recursive'=>2,'conditions'=>$ec,
    			'limit' => 100,
    ...
    public function lab_gen_page() {
        ...
        if ($this->request->is('ajax') && !empty($this->data)) {
            ...
            if(!empty($page)){
                ...
                if(!empty($chk)){ echo "<div class='alert alert-danger fadeIn animated'>Select language page already exists.</div>"; }
                else {
            ...
            }else{ echo "<div class='alert alert-danger fadeIn animated'>Please try again later</div>"; }
        }
    ...
    public function lab_lang_car_detail($id = null,$lang = null) {
        ...
        if( !empty( $id ) && !empty($lang) ){
            ...
            if(!empty($id)){
                $cList = $sList = array();
                $this->ItemDetail->bindModel(array('hasMany'=>array('QualityDetail'),'belongsTo'=>array('Brand','Model','Motor')));
                ...
                if(isset($q['tab']) && $q['tab'] == 'images'){
                    ...
                    $sliders = $this->Library->find('all',array('conditions'=>array('Library.id'=>$ids),
                        'order'=>$or));
                    
                    $gallery = $this->Library->find('all',array('conditions'=>array('Library.id'=>$gids)));
                }
               $langArr = $this->Language->find('list',array('fields'=>array('Language.code','Language.language')));
                
                if(empty($data)){ $this->layout = 'lab_404';}
            }
            
            $this->set(compact('data','q','gallery','sliders','cList','sList','sData','langArr','lang'));
        }
    ...
    public function lab_change_positions_model() {
        ...
        if($this->RequestHandler->isAjax()){
           if(isset($this->data['model']) && !empty($this->data['model'])){
               $arr = [];
               foreach($this->data['model'] as $key => $val) { $arr[] = ['id'=>$val,'pos'=>$key]; }
               if(!empty($arr)){ $this->Model->saveAll($arr); }
             }
        }
    }
    
    public function lab_change_positions_pic( $id = null) {
        $this->autoRender = false;
        if ($id == 'ss'){
            if($this->RequestHandler->isAjax()){
                if(isset($this->data['ss']) && !empty($this->data['ss'])){
                    $arr = [];
                    foreach($this->data['ss'] as $key => $val) { $arr[] = ['id'=>$val,'pos'=>$key]; }
                    if(!empty($arr)){ $this->Library->saveAll($arr); }
                }
            }
        }
        ...
        elseif ($id == 'tt'){
            if($this->RequestHandler->isAjax()){
                if(isset($this->data['tt']) && !empty($this->data['tt'])){
                    $arr = [];
                    foreach($this->data['tt'] as $key => $val) { $arr[] = ['id'=>$val,'pos'=>$key]; }
                    if(!empty($arr)){ $this->Library->saveAll($arr); }
                }
            }
        }
        ...
        elseif ($id == 'gal'){
            if($this->RequestHandler->isAjax()){
                if(isset($this->data['gal']) && !empty($this->data['gal'])){
                    $arr = [];
                    foreach($this->data['gal'] as $key => $val) { $arr[] = ['id'=>$val,'pos'=>$key]; }
                    if(!empty($arr)){ $this->Library->saveAll($arr); }
                }
            }
        }
    ...
    public function lab_change_positions() {
        ...
        if($this->RequestHandler->isAjax()){
            if(isset($this->data['recordsArray']) && !empty($this->data['recordsArray'])){
                $arr = [];
                foreach($this->data['recordsArray'] as $key => $val) { $arr[] = ['id'=>$val,'pos'=>$key]; }
                if(!empty($arr)){
    ...
    public function lab_update_car_detail($id = null) {
        ...
        if( !empty($id) && isset($this->request->query['lng_act']) && !empty($this->request->query['lng_act'])){
            ...
            if(!empty($d)){
                $st = 1;
                if($d['ItemDetail']['status'] == 1){ $st = 0;}
                elseif($d['ItemDetail']['status'] == 0){ $st = 1;}
        ...
        if(isset($this->data['Shipping']) && !empty($this->data['Shipping'])){
            ...
            if( $c == 0){
                if(!empty($this->data['Shipping']['type1'])){
                    $arr[] = array('type'=>2,'item_detail_id' => $this->data['Shipping']['item_detail_id'],'brand_id'=> $this->data['Shipping']['brand_id'],'model_id'=> $this->data['Shipping']['model_id'],
                ...
                if(!empty($this->data['Shipping']['type2'])){
                    $arr[] = array('type'=>3,'item_detail_id' => $this->data['Shipping']['item_detail_id'],'brand_id'=> $this->data['Shipping']['brand_id'],'model_id'=> $this->data['Shipping']['model_id'],
                        'motor_id'=> $this->data['Shipping']['motor_id'],'country_id'=> $this->data['Shipping']['country'],'state_id'=> $this->data['Shipping']['state'],
                ...
                if(!empty($this->data['Shipping']['type3'])){
                    $arr[] = array('type'=>1,'item_detail_id' => $this->data['Shipping']['item_detail_id'],'brand_id'=> $this->data['Shipping']['brand_id'],'model_id'=> $this->data['Shipping']['model_id'],
                        'motor_id'=> $this->data['Shipping']['motor_id'],'country_id'=> $this->data['Shipping']['country'],'state_id'=> $this->data['Shipping']['state'],
                        'for_customer' =>$this->data['Shipping']['type2']['for_customer'],'for_dealer' => $this->data['Shipping']['type2']['for_dealer']);
                }
        ...
        if(isset($this->data['ItemDetail']) && !empty($this->data['ItemDetail'])){
            ...
            if( $this->ItemDetail->save($this->data) ){
                $this->Session->setFlash(__('Saved'), 'default', array('class' => 'alert alert-success'), 'msg');
            }else{ $this->Session->setFlash(__('Not Saved. Please check SEO URL. it should be unique.'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
            $this->redirect($st);
        }
        $q = $this->request->query;
        if(!empty($id)){
            ...
            if(isset($q['tab']) && $q['tab'] == 'images'){
                $ids = explode(',', $data['ItemDetail']['slider']);
                $tt_ids = explode(',', $data['ItemDetail']['tt_slider']);
                $gids = explode(',', $data['ItemDetail']['gallery']);
                $or = $or1 = array();
                if(!empty($data['ItemDetail']['slider'])){ $or = array('FIELD(Library.id,' . $data['ItemDetail']['slider'] . ')'); }
                if(!empty($data['ItemDetail']['tt_slider'])){ $or1 = array('FIELD(Library.id,' . $data['ItemDetail']['tt_slider'] . ')'); }
                $sliders = $this->Library->find('all',array('conditions'=>array('Library.id'=>$ids), /*'order'=>$or*/ 'order'=>['Library.pos'=>'ASC'] ));
                //ec($sliders);die;
                $slidersTT = $this->Library->find('all',array('conditions'=>array('Library.id'=>$tt_ids),/* 'order'=>$or1 */ 'order'=>['Library.pos'=>'ASC']));
                $gallery = $this->Library->find('all',array('conditions'=>array('Library.id'=>$gids),'order'=>['Library.pos'=>'ASC']));
            }
            if( isset($q['tab']) && in_array($q['tab'], array('shipping','manage_shipping')) ){
                ...
                if(!empty($q['cid'])){ $sList = $this->World->find('list',array('conditions'=>array('World.type'=>'st','World.in_location'=>$q['cid'],'World.status'=>1),'order' => array('World.name' =>'ASC'),'field'=>array('id','name'))); }
                if( $q['tab'] == 'manage_shipping'){
                    ...
                    if(isset($q['cid']) && isset($q['sid'])){ $sC = array('Shipping.item_detail_id'=>$id, 'Shipping.country_id'=>$q['cid'],'Shipping.state_id'=>$q['sid']); }
                    else{ $sC = array('Shipping.item_detail_id'=>$id); }
            ...
            if( isset( $q['tab']) &&  $q['tab'] == 'multilingual'){
                ...
                if(!empty($langArr)){
                    foreach ($langArr as $lk=>$lv){ $lgcode[] = $lk; }
                }
    ...
    public function _xls_to_pro($file = null){
    	...
    	if(!empty($file)){
    		...
    		if(isset($data->sheets[0]['cells'][2]) && !empty($data->sheets[0]['cells'][2])){
    			...
    			}else{ echo "<div class='alert alert-danger'>inventory record not found</div>"; }
    		...
    		foreach ($data as $list){
    			...
    			if(!empty($cat_back)){
    				foreach ($cat_back as $caList){
    					$Arr[]= array('id'=>$caList['Product']['id'],
    	...
    	if(!empty($id)){
    		...
    		if(!empty($data) && !empty($w)){
	    		...
	    		foreach ($w as $list){
	    			...
	    			$wList[] = array('id'=>'NULL','type'=>1,'Product_type'=>'Catback','item_detail_id'=>$data['ItemDetail']['id'],'brand_id'=>$data['ItemDetail']['brand_id'],'model_id'=>$data['ItemDetail']['model_id'],'motor_id'=>$data['ItemDetail']['motor_id'],
	    			...
	    			$wList[] = array('id'=>'NULL','type'=>2,'Product_type'=>'DP','item_detail_id'=>$data['ItemDetail']['id'],'brand_id'=>$data['ItemDetail']['brand_id'],'model_id'=>$data['ItemDetail']['model_id'],'motor_id'=>$data['ItemDetail']['motor_id'],
	    					'for_dealer'=>0,'for_customer'=>0,'country_id'=>$list['Country']['id'],'country_name'=>$list['Country']['name'],'state_id'=>$list['World']['id'],'state_name'=>$list['World']['name']);
	    			$wList[] = array('id'=>'NULL','type'=>3,'Product_type'=>'Box','item_detail_id'=>$data['ItemDetail']['id'],'brand_id'=>$data['ItemDetail']['brand_id'],'model_id'=>$data['ItemDetail']['model_id'],'motor_id'=>$data['ItemDetail']['motor_id'],
	    					'for_dealer'=>0,'for_customer'=>0,'country_id'=>$list['Country']['id'],'country_name'=>$list['Country']['name'],'state_id'=>$list['World']['id'],'state_name'=>$list['World']['name']);
    ...
    public function lab_create_car_page($eid=null,$mid=null,$bid=null){
    	...
    	if( !empty($eid) && !empty($mid) && !empty($bid) ){
    		...
    		if(!empty($i)){ $this->redirect(SITEURL.'lab/labs/update_car_detail/'.$i['ItemDetail']['id']); }
    		else{
    ...
    public function lab_all_promo($id = null,$st = null) {
    	...
    	if(!empty($id) && !empty($st)){
    		...
    		if(!empty($d)){
    			$status = 1;
    			if($d['PromoCode']['status'] == 1){ $status = 0;}
    				...
    				if(isset($this->data['ExhaustProduct']['id'])){
    					...
    					if(isset($this->data['ExhaustProduct']['new_image']) && !empty($this->data['ExhaustProduct']['new_image'])){
    						...
    						if(!empty($img)){
    							...
    							if(empty($c)){ echo $s; echo "<div class='alert alert-danger fadeIn animated'><strong>Error</strong> Image not found. please try to use another image</div>"; exit;  }
    							else{
    							...
    							if( isset($get_b_id['ExhaustModel']['exhaust_brand_id']) && !empty($get_b_id['ExhaustModel']['exhaust_brand_id']) ){
    								...
    								echo $s;  echo '<div class="alert alert-danger fadeIn animated">Sorry, this is not working at the moment. Please try again later.</div>'; exit;
    							}
    							...
    							if ($this->ExhaustProduct->validates() ) {
    								...
    								echo $s; echo '<script>window.location = "'.SITEURL.'lab/labs/exhaust_product";</script>';
    					...
    					if(!empty($c)){
    						...
    						if ($this->ExhaustProduct->validates() ) {
    							...
    							if(!empty($errors)){
    								foreach ($errors as $err){ $str.=$err[0]."<br>"; }
    								echo $s;  echo '<div class="alert alert-danger fadeIn animated">'.$str.'</div>'; } }
    					}else{ echo $s; echo "<div class='alert alert-danger fadeIn animated'><strong>Error</strong> Image not found. please try to use another image</div>"; exit; }
    				}
    ...
    public function lab_exhaust_product_advance() {
    	if ($this->request->is('ajax')) {
    		...
    		$searchData = $this->request->data['name_value'];
    		$this->paginate = array('recursive' => 1,
    				'conditions' => array( 'Product.type'=>array(2,3,5),
    						'or' => array(
    ...
    public function lab_exhaust_product() {
    	...
    	$q = $this->request->query;
    	$brand = $this->Brand->find('list',array('order'=>array('Brand.name'=>'ASC'),'fields'=>array('id','name')));
    	if(isset($q['brand']) && !empty($q['brand'])){ $c = array('Model.brand_id'=>$q['brand']); $ec[]= array('Product.brand_id'=>$q['brand']); }
    	$model_list = $this->Model->find('list',array('conditions'=>$c, 'order'=>array('Model.name'=>'ASC'),'fields'=>array('id','name')));
    	if( isset($q['model']) && !empty($q['model'])){ 
    		$engList= $this->Motor->find('list',array('conditions'=>array('Motor.model_id'=>$q['model']),'order'=>array('Motor.name'=>'ASC'),'fields'=>array('id','name')));
    		$ec[]= array('Product.model_id'=>$q['model']); }
 
    	if( isset($q['motor']) && !empty($q['motor'])){ $ec[]= array('Product.motor_id'=>$q['motor']); }
    	
    	
    	$this->Product->bindModel(array('belongsTo'=>array('Library','Brand','Model','Motor')));
    	$this->paginate = array( 'conditions'=>$ec,
    			'maxLimit' =>30,'limit' =>30,'order' => array('Product.id' => 'DESC'));
    	$data = $this->paginate('Product');

    	 
    	$paging = $this->params['paging'];
    	$this->set(compact('data','paging','brand','model_list','q'));
    ...
    public function lab_exhaust_model() {
    	...
    	if(isset($this->request->query['id']) && !empty($this->request->query['id'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['ExhaustModel']['status'] == 1){ $st = 0;}
    			elseif($d['ExhaustModel']['status'] == 0){ $st = 1;}
    ...
    public function lab_add_exhaust_model($id=null) {
    	...
    	if(!empty($id)){
    		...
    		if(!empty($data)){ $list_model = $this->Model->find('list',array('conditions'=>array('Model.brand_id'=>$data['ExhaustModel']['brand_id']))); }
    				...
    				if(isset($this->data['ExhaustModel']['new_image']) && !empty($this->data['ExhaustModel']['new_image'])){
    					...
    					if(!empty($img)){
    						...
    						if ($this->ExhaustModel->validates() ) {
    							...
    							$this->Session->setFlash(__('Brand has been successfully saved'), 'default', array('class' => 'alert alert-success'), 'msg');
    							echo $s; echo '<script>window.location = "'.SITEURL.'lab/labs/exhaust_model";</script>';
...
public function lab_add_cars($id=null) {
    	...
    	if(!empty($id)){ $data = $this->ExhaustBrand->find('first',array('conditions'=>array('ExhaustBrand.id'=>$id))); }
    			...
    			if(isset($this->data['ExhaustBrand']['new_image']) && !empty($this->data['ExhaustBrand']['new_image'])){
	    			...
	    			if(!empty($img)){
	    				...
	    				if ($this->ExhaustBrand->validates() ) {
	    					...
	    					$this->Session->setFlash(__('Brand has been successfully saved'), 'default', array('class' => 'alert alert-success'), 'msg');
	    					echo $s; echo '<script>window.location = "'.SITEURL.'lab/labs/cars";</script>';
    ...
    public function lab_cars() {
    	...
    	if(isset($this->request->query['id']) && !empty($this->request->query['id'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['ExhaustBrand']['status'] == 1){ $st = 0;}
    			elseif($d['ExhaustBrand']['status'] == 0){ $st = 1;}
    		...
    		if(!empty($this->data)){
    			...
    			if(!empty($data)){
    				...
    				if(!empty($title)){
    					...
    					if($is_r > 0){ echo $s; echo "<div class='alert alert-danger fadeIn animated'><strong>Error</strong> Image name already exists. please try to use another name</div>"; exit;}
    					else{
    				...
    				echo $s; echo "<div class='alert alert-success fadeIn animated'>Saved</div>";
    			}
    			else{ echo $s; echo "<div class='alert alert-danger fadeIn animated'><strong>Error</strong> Please try again later.</div>"; }
    ...
    public function lab_new_up(){
        $this->autoRender =false;
         ...
         if( isset($_FILES) && !empty($_FILES)){
             $folder = 'all';
             if(!empty($_POST['action'])){ $folder = $_POST['action']; }
             if (!file_exists('cdn/'.$folder)) { mkdir('cdn/'.$folder, 0777, true); }
    ...
    public function lab_media() {
    	...
    	if($this->RequestHandler->isAjax()){
    		...
    		if(!empty($this->data)){
    			...
    			else{ $folder = strtolower(Inflector::slug($this->data['Library']['folder'], '-')); }
    			...
    			}else{ echo "<div class='alert alert-danger fadeIn animated'>Please select files</div>"; }
    		}
    		exit;
    	}
    ...
    public function lab_api_country(){
    	...
    	if($this->RequestHandler->isAjax()){
    		...
    		if(isset($this->data['type']) && $this->data['type'] == 'country'){
    			...
    			}else{ echo "<div class='alert alert-danger fadeIn animated'>This Country name is already exist.</div>"; }
    		}
    ...
    public function lab_state($id = null) {
    	...
    	if(isset($this->request->query['status']) && !empty($this->request->query['status'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['World']['status'] == 1){ $st = 0;}
    			elseif($d['World']['status'] == 0){ $st = 1;}
    			$arr = array('id'=>$d['World']['id'],'status'=>$st);
    			$this->World->save($arr);
    		}
    	...
    	if(!empty($this->data)){
    		...
    		else { $this->Session->setFlash(__('State has no tbeen added'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
    	}
    	...
    	if(isset($this->request->query['edit']) && !empty($this->request->query['edit'])){
    		...
    		if(empty($all)){ $this->layout = 'lab_404'; }
    ...
    public function lab_edit_shipping( $id = null ) {
        if(!empty($id)){
            ...
            if ($this->request->is('get')) {
                ...
                if (!empty($e)) { $this->request->data = $e; }
                else { $this->redirect(array('controller' => 'labs', 'action' => 'shipping')); }
            } else {
    ...
    public function lab_country($id = null){
    	...
    	if(isset($this->request->query['cc']) && !empty($this->request->query['cc'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['World']['is_cc'] == 1){ $st = 0;}
    			elseif($d['World']['is_cc'] == 0){ $st = 1;}
    	...
    	if(isset($this->request->query['for_dealer']) && !empty($this->request->query['for_dealer'])){
    		...
    		if(!empty($d)){
    			$st = 1;
    			if($d['World']['for_dealer'] == 1){ $st = 0;}
    			elseif($d['World']['for_dealer'] == 0){ $st = 1;}
    	...
    	if(!empty($this->data)){
    		...
    		if( empty($this->data) && $c > 0){ $this->Session->setFlash(__('country name already exist'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
    		else{
    		...
    		else { $this->Session->setFlash(__('Country has no tbeen added'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
    		}
    ...
    public function lab_my_email_template($id = null) {
    	...
    	if(!empty($id)){
    		...
    		if ($this->request->is('get')) {
    			...
    			if (!empty($e)) { $this->request->data = $e; }
    			else { $this->redirect(array('controller' => 'labs', 'action' => 'email_templates')); }
    		} else {
    ...
    public function lab_status_email_templates($id=null,$t=null){
    	...
    	if(!empty($id)){
    		...
    		if(!empty($d)){
    			if($d['EmailTemplate']['status'] == 1){ $s = 0; }else{ $s = 1; }
    ...
    public function lab_settings() {
    	...
    	if (empty($this->data)) {
    		...
    		$this->request->data = $this->WebSetting->read();
    			...
    			{ $this->Session->setFlash(__('Saved'), 'default', array('class' => 'alert alert-success'), 'msg'); }
    			else{ $this->Session->setFlash(__('not savedd'), 'default', array('class' => 'alert alert-danger'), 'msg'); }
    ...
    public function lab_fee(){
    	...
    	if(!empty($this->data)){
    	if ($this->Fee->save($this->request->data)) { $this->Session->setFlash(__('Fees has been updated.'), 'default', array('class' => 'alert alert-success'), 'msg'); } 
    	else { $this->Session->setFlash(__('Unable to update fess.'), 'default', array('class' => 'alert alert-danger '), 'msg'); }
    ...
    public function lab_update_level(){
    	...
    	if(!empty($this->data)){
    		...
    		if(!empty($d)){
    			if(! in_array($this->data['level'], array(1,2,3))){ echo '<div class="alert alert-danger">Please try again later</div>'; }
    			elseif($d['User']['status'] != 1){ echo '<div class="alert alert-danger">User is not active.</div>'; }
    			elseif($d['User']['dealer_level_id'] == $this->data['level']){ echo '<div class="alert alert-danger">This Dealder already have this membership, Please select different membership level.</div>'; }
    			else{
    				...
    				$parameters = array('NAME' => $d['User']['first_name']);
    				 $tname = null;
    				 if( $this->data['level'] == 1){ $tname = 'GoldLevel'; }
    				 elseif( $this->data['level'] == 2){ $tname = 'SilverLevel'; }
    				 elseif( $this->data['level'] == 3){ $tname = 'MasterLevel'; }
    		...
    		}else{ echo '<div class="alert alert-danger">Please try again later</div>';}
    		//$this->data['level']
    	}
    }
    ...
    public function lab_up_user_st(){
    	...
    	if(!empty($this->data)){
    		...
    		if(!empty($d)){
    			$st = $t = null;
    			if(in_array($d['User']['status'],array(0,2,3))){ $st = 1; $t = '<p class=\"text-green\">Active</p>'; }
    			elseif( $d['User']['status'] == 1 ){ $st = 3; $t = '<p class="text-red">Inactive</p>';}
    ...
    public function lab_up_user_visibility(){
    	...
    	if(!empty($this->data)){
    		...
    		if(!empty($d)){
    			$st = $t = null;
    			if(in_array($d['User']['is_hide'],array(0,2,3))){ $st = 1; $t = '<p class=\"text-green\">Visible</p>'; }
    			elseif( $d['User']['is_hide'] == 1 ){ $st = 2; $t = '<p class="text-red">Hidden</p>';}
    ...
    public function lab_user_profile($id= null) {
    	...
    	if(!empty($id)){
    		...
    		if(!empty($data)){ $this->set('title_for_layout', $data['User']['first_name'].'\'s Profile : '.WEBTITLE); }
    		$this->set('data',$data);
    	}
    }
    		...
    		if($this->data['type'] == 'password'){
    			...
    			if(isset($this->data['id']) && !empty($this->data['id'])){
    				if(!empty($this->data['new_pwd']) && strlen($this->data['new_pwd']) >=6 ){
    					...
    					$this->User->save($arr);
    					echo $s; echo "<div class='alert alert-success fadeIn animated'>Password has been changed successfully</div>";
    				}else{ echo $s; echo "<div class='alert alert-danger fadeIn animated'>Password must have minimum 6 characters</div>"; }
    			}
    		}
    ...
    public function lab_rotate_pic($id=null,$type = null){
    	...
    	if(!empty($this->data)){
    		...
    		if(file_exists($this->data['path'])){
    			...
    			if($ty == 'anti_clock'){ $degrees = 90; }
    			elseif($ty == 'clock'){ $degrees = -90; }
    			else{ $degrees = 90; }
    ...
    public function lab_add_media($id = null,$type = null,$for = null) {
    	if ($this->request->is('ajax')) {
    		$aj = 'yes';
    		$q = $this->request->query;
    		$flist = $this->Library->find('list',array('group'=>array('Library.folder'),'fields'=>array('id','folder')));
    		$this->set(compact('flist','aj','id','type','q','for'));
    	}else{
    		$this->layout = false;
    	}
    }
    public function lab_get_folder_img($f = null,$type = null) {
    	$this->layout = false;
    	if(!empty($f)){
    		$this->paginate = array(
  ...
  public function lab_msg_pop(){
	  	$msg = null;
	  	if(isset($_GET['m']) && !empty($_GET['m'])){ $msg = urldecode($_GET['m']); }
	  	if(isset($_GET['r']) && !empty($_GET['r'])){ $this->set('reload','here'); }
	  	$this->set('msg',$msg);
  }
  	...
  	if ($this->RequestHandler->isAjax() && !empty($this->data)) {
  		if(!empty($this->data['folder'])){
  			...
  			if(!empty($files)){
  				...
  				if($tot > 0){ echo "<div class='alert alert-success'>Total $tot files has been uploaded.</div>"; }
  				else{ echo "<div class='alert alert-danger'>Images not uploaded.</div>"; }
  			}else{ echo "<div class='alert alert-danger'>Selected Folder is empty.</div>";}
  		}else{ echo "<div class='alert alert-danger'>Please select folder.</div>"; }
  		exit;
  	}
  ...
  public function lab_manage_slider($id = null) {
  	...
  	$this->HomeSlider->bindModel(array('belongsTo'=>array('Library')));
  	
  	if(!empty($id)){
  		$e = $this->HomeSlider->find('first',array('conditions'=>array('HomeSlider.id'=>$id)));
  		$this->set('e',$e);
  	}
  	
  	if ($this->RequestHandler->isAjax() && !empty($this->data)) {
  		$this->HomeSlider->save($this->request->data);
  ...
  public function lab_home_slider() {
  	...
  	if(isset($this->request->query['del']) && !empty($this->request->query['del'])){
  		$this->HomeSlider->id = $this->request->query['del'];
  		$this->HomeSlider->delete();
  		$this->redirect('/lab/labs/home_slider');
  	}
  	
  	$this->paginate = array('maxLimit' =>50,'limit' =>50,'order' => array('HomeSlider.pos' => 'ASC'));
  	$data = $this->paginate('HomeSlider');
  	$this->set('data', $data);
    ...
    public function lab_home_slider_sort() {
    	...
    	if(isset($this->data['img']) && !empty($this->data['img'])){
    		foreach($this->data['img'] as $key => $val) {
    			 ...
    			 $arr['pos'] = $key;
    			 $d[] = $arr;}
    			if(!empty($d)){  if( $this->HomeSlider->saveMany($d, array('deep' => false)) ){ echo "<script>console.log('updated( ".count($d)." )');</script>";} }
    	...
    	if(!empty($this->data)){
    		...
    		if(isset($this->data['img']) && !empty($this->data['img'])){
    			foreach($this->data['img'] as $key => $val) {
    				...
    				$arr['pos'] = $key;
    				$d[] = $arr;}
    				if(!empty($d)){  if( $this->NewRelease->saveMany($d, array('deep' => false)) ){ echo "<script>console.log('updated( ".count($d)." )');</script>";} }
    ...
    public function lab_update_bg($id = null) {
        ...
        if ($this->RequestHandler->isAjax() && !empty($this->data)) {
            ...
            if (!file_exists('cdn/bg_image')) { mkdir('cdn/bg_image', 0777, true); }
            
            if( empty($this->data['BgImage']['img']['name']) ){ echo '<div class="alert alert-danger" role="alert">Please select an image.</div>'; }
            else{
    ...
    public function lab_languages() {
        ...
        if(isset($this->request->query['dis']) && !empty($this->request->query['dis'])){
            ...
            if(!empty($d)){
                $st = 1;
                if($d['Language']['status'] == 1){ $st = 2;}
                elseif($d['Language']['status'] == 2){ $st = 1;}
        ...
        if( $this->request->is('get') &&  isset($this->request->query['edit']) && !empty($this->request->query['edit'])){
            ...
            if( !empty($d) ) { $this->request->data = $d; }
        ...
        if ($this->RequestHandler->isAjax() && !empty( $this->data )) {
            ...
            if ($this->Language->validates() === true) {
                ...
                if ($this->Language->save($this->request->data)) { echo "<div class='alert alert-success'>Saved</div>"; }
                else { echo "<div class='alert alert-danger'>Not Saved</div>"; }
                ...
                if (!empty($failed_fields)) {
                    foreach ($failed_fields as $k=>$lf) { $er .= "<p><strong>".ucwords(str_replace('_',' ', $k)).": </strong> $lf[0]</p>";  }
