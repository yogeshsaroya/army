...
echo $this->Form->hidden('type',array('value'=>'co'));
?>                    
                    
<div class="col-md-12"><div class="box box-success"><div class="box-body"><div class="row">
<div class="col-xs-12"><div class="form-group"><?php echo $this->Form->input('World.name',array('type'=>'text','maxlength'=>50,'class'=>'form-control','label'=>'Country name','required'=>true));?></div></div>
</div></div></div></div>
...
<?php echo $this->Form->end();?>                    
              ...
              <div class="box box-success">
                <div class="box-header"> <h3 class="box-title">All Country <?php if(!empty($data)){ echo "(".count($data).")";}?></h3> </div>
                <div class="box-body">
                  								<div class="row">
...
if (!empty($data)) {
    ...
    foreach ($data as $list) { ?>
        <tr class="odd gradeX">
           ...
           <td class="center gnTxt"> <?php if($list['World']['for_dealer'] == 1){ echo $this->html->link('Active','/lab/labs/country?for_dealer='.$list['World']['id'],array('class' => 'text-green')); }
                        else{ echo $this->html->link('Deactive','/lab/labs/country?for_dealer='.$list['World']['id'],array('class' => 'text-red')); }?></td>
           
           <td class="center gnTxt"> <?php if($list['World']['is_cc'] == 1){ echo $this->html->link('Active','/lab/labs/country?cc='.$list['World']['id'],array('class' => 'text-green')); }
                        else{ echo $this->html->link('Deactive','/lab/labs/country?cc='.$list['World']['id'],array('class' => 'text-red')); }?></td>
           


           <td class="center gnTxt"> <?php echo $this->html->link('Delete','/lab/labs/country?del='.$list['World']['id'],array('class' => 'text-red','confirm' => 'Do you want to delete this state?'));?></td>
           
           <td class="center gnTxt"> <?php if($list['World']['status'] == 1){ echo $this->html->link('Active','/lab/labs/country?status='.$list['World']['id'],array('class' => 'text-green')); }
                        else{ echo $this->html->link('Deactive','/lab/labs/country?status='.$list['World']['id'],array('class' => 'text-red')); }?></td>
                        
			<td class="center gnTxt"> <?php echo $this->html->link('Add','/lab/labs/state/'.$list['World']['id'],array('class' => ''));?></td>                        
           </tr>
	<?php $n++;} }else{ ?>
