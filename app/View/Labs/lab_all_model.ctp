	...
	<section class="content">
		...
		<div class="box">
			<div class="box-body">
				...
				<div class="col-md-12">
					<?php
					if (isset($e['Model']['brand_id']) && !empty($e['Model']['brand_id'])) {
						$id = $e['Model']['brand_id'];
					} ?>
						...
						<div class="box-body">
							<div class="row">
								...
								<div class="col-md-4"> <label for="WorldName">Model name</label>
									...
									<input id="name" type="text" class="form-control" name="name" value="<?php if (isset($e['Model']['name'])) {
																												echo $e['Model']['name'];
																											} ?>" placeholder="Model name">
									<input type="hidden" name="id" id="id" value="<?php if (isset($e['Model']['id'])) {
																						echo $e['Model']['id'];
																					} ?>">
								...
								<div class="col-md-4">
									<label for=""> &nbsp;&nbsp; </label>
									<input type="button" value="Save" id="add_br" name="ct" class="btn btn-primary form-control">
								</div>
							</div>
							<div id="app_err"> </div>
						</div>
									...
									<table class="table table-bordered table-hover dataTable" id="example2" role="grid" aria-describedby="example2_info">
										<thead>
											...
											<tr role="row">
												...
												<th class="text-center"><?php echo $this->Paginator->sort('Model.motor_count', 'Motor #', array('escape' => false)); ?></th>
											...
											<?php if (!empty($model)) {
												...
												foreach ($model as $list) { ?>
													<tr id="model_<?php echo $list['Model']['id']; ?>">
														...
														<td><?php echo $this->html->link($list['Model']['name'], '/lab/labs/all_motor/'.$list['Model']['brand_id'].'/'. $list['Model']['id']);?></td>
														<td> <?php echo $this->html->link($list['Brand']['name'], '/lab/labs/all_make');?></tb>
														<td class="text-center"> <?php echo $this->html->link($list['Model']['motor_count'], '/lab/labs/all_motor/'.$list['Model']['brand_id'].'/'. $list['Model']['id']);?></tb>
														...
														<td> <?php
																if ($list['Model']['status'] == 1) {
																	echo $this->html->link('Active', '/lab/labs/all_model/?status=' . $list['Model']['id'], array('class' => 'text-green'));
																} else {
																	echo $this->html->link('Deactive', '/lab/labs/all_model/?status=' . $list['Model']['id'], array('class' => 'text-red'));
																}

																?></tb>
																<td> <a href="javascript:void(0);" onclick="model_del(<?php echo $list['Model']['id']; ?>);">Delete</a></td>
													</tr>
												<?php $n++;
												}
											} else { ?>
...
<script>
		...
		var txt;
		var r = confirm("Are you sure you want to delete Car Model? related motor and car will be deleted.");
		if (r == true) {
			$.ajax({
				type: 'POST',
				url: '' + SITEURL + 'lab/labs/del_model',
				data: { id: id },
