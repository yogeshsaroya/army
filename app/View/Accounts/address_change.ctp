...
<style>
select{background: #e2e3e4!important;border: none !important;}
</style>
...
if(isset($edit_add) && !empty($edit_add)){
	$this->request->data['Address'] = $edit_add['Address'];
...
<div class="col-sm-10"><?php echo $this->Form->input('Address.country_list_id',array('options'=> $this->Lab->getCountryNew(),'label'=>false,'div'=>false,'empty'=>' --- Please Select --- ', 'class'=>'form-control','required'=>"required"));?></div></div>

<div class="form-group required"><label class="col-sm-2 control-label" for="input-firstname">Region / State</label> 
<div class="col-sm-10"><?php echo $this->Form->input('Address.state_name',array('type'=>'text','placeholder'=>'Region / State','label'=>false,'div'=>false, 'class'=>'form-control','required'=>"required"));?></div></div>

<?php if(isset($edit_add) && !empty($edit_add)){}else{?>
