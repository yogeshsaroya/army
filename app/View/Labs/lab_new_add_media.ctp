...
<div class="col-sm-1 col-md-2 lib_folder" onclick="load('<?php echo $v;?>','<?php if(isset($q['one'])){ echo "s";}else{ echo "m";}?>');"><div class="thumbnail">
...
<input type="hidden" value="<?php echo $e;?>" id="e">
...
function cls(){ $.magnificPopup.close();}
...
function load(folder,type){
	if(id !=''){
		...
		$('.lib_folder').show();
	$.ajax({type: 'POST',
		url: '<?php echo SITEURL."lab/labs/new_get_folder_img/";?>'+folder+'/'+type,
		data: 'id='+folder+'',
