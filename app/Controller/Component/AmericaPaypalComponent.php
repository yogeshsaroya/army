...
class AmericaPaypalComponent extends Component {
...
// Live paypal API config
	...
	public $config = array(
		...
		'password' => '8VQ44UUCMQQPRC77',
		'email' => 'sales_api1.vividracing.com',
		'signature' => 'A8ExWiKObwE3Gs6ixIPP7gvEO3y8AHuJx3mfEGAWsVRuZkHTPHw5-AjR'
	);
	...
	public $sandboxConfig = array(
        ...
        'password' => 'FZGTVW38YBV9SBHX',
        'email' => 'info_api1.smartstayz.in',
        'signature' => 'AFcWxV21C7fd0v3bYYYRCpSSRl31A1jwDsmngtr7kJkp7240jHcD3WGU'
    );
	// API version
	...
	public function doExpressCheckoutPayment() {
		...
		parse_str($response , $parsed);
		// Return the token, or throw a human readable error
		if(array_key_exists('TOKEN', $parsed) && array_key_exists('ACK', $parsed) && $parsed['ACK'] == 'Success') { return $parsed; }
		elseif(array_key_exists('ACK', $parsed) && array_key_exists('L_LONGMESSAGE0', $parsed) && $parsed['ACK'] != 'Success') {
			$this->log($parsed , 'paypal');	
			throw new Exception($parsed['ACK'] . ' : ' . $parsed['L_LONGMESSAGE0']);
		}
